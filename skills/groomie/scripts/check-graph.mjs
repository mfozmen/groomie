...
const EDGE_KINDS = new Set(['blocks']);

// Returns an array of violation strings ([] = valid). `md` is optional raw markdown;
// when given, blocking links in the prose are cross-checked against the graph.
...
// Parse the markdown's blocking prose and compare it to the graph edges.
...
function selfTest() {
  ...
  const md = [
    ...
    '### B1 — broken', '',
    '**Reproduce Steps**', '1. do the thing', '',
    '**Expected Behaviour**', 'it works', '',
    '**Actual Behaviour**', 'it breaks',
  ].join('\n');
  ...
  assert(checkGraph({ nodes: good.nodes, edges: [{ source: 'B1', target: 'S1', kind: 'affects' }] })
    .some(x => /bad kind/.test(x)), 'affects edge kind now rejected as unknown');
