...
- The breakdown must already exist: `<ISSUE-KEY>/<ISSUE-KEY>-groomed.md` **and**
  `<ISSUE-KEY>/<ISSUE-KEY>-groomed.json` under the per-issue folder (or the legacy flat
  `<ISSUE-KEY>-groomed.{md,json}` in the working directory, for pre-folder outputs). If neither is
  found, tell the user to run `/groomie <ISSUE-KEY>` first — do **not** silently groom a fresh one.
